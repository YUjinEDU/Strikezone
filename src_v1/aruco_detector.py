...
class ArucoDetector:
    ...
    def project_points(self, points_3d, rvec, tvec):
        ...
        try:
            ...
            flat = projected_points.reshape(-1, 2).astype(np.float32)
            if np.any(~np.isfinite(flat)):
                print("Warning: NaN or Inf values detected in projection")
                flat = np.zeros((len(points_3d), 2), dtype=np.float32)
            return flat
        except Exception as e:
            print(f"투영 오류: {e}")
            return np.zeros((len(points_3d), 2), dtype=np.float32)
    ...
    def draw_impact_point_on_plane2(self, frame, point_3d, plane_corners, rvec, tvec, 
        ...
        try:
            ...
            # 좌표를 순수 Python int로 변환 (OpenCV 호환)
            cx, cy = pts_2d[0]
            if not (np.isfinite(cx) and np.isfinite(cy)):
                return
            center_2d = (int(round(float(cx))), int(round(float(cy))))
