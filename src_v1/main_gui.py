...
class PitchAnalyzer(QObject):
    ...
    def _reset_state(self):
        ...
        # 판정 효과 (테두리 그라데이션)
        self.judgment_effect_type = None  # 'strike', 'ball', 'out'
        self.judgment_effect_start_time = 0.0
        self.judgment_effect_duration = 1.5  # 효과 지속 시간 (초)
        self.game_result = None  # 게임모드 결과 {'success': bool, 'target': int, 'actual': int}
    ...
    @staticmethod
    def _to_cv_point(point):
        """OpenCV 드로잉용 안전한 좌표 변환"""
    ...
    @staticmethod
    def _to_int_poly(points):
        """다각형 좌표 배열을 np.int32로 변환 (NaN 차단)"""
        if points is None:
            return None
    ...
    def process_frame(self, frame):
        ...
        for rvec, tvec in zip(rvecs, tvecs):
            ...
            if self.vis_settings.get('zone', True):
                ...
                front_poly = self._to_int_poly(projected_front)
                if front_poly is not None:
                    cv2.polylines(overlay_frame, [front_poly], True, (0, 255, 0), 2)
                # 뒷면 (plane2)
                projected_back = self.aruco_detector.project_points(self.ball_zone_corners2, rvec, tvec)
                back_poly = self._to_int_poly(projected_back)
                if back_poly is not None:
                    cv2.polylines(overlay_frame, [back_poly], True, (0, 255, 0), 2)
                # 앞뒤 연결선 (4개 모서리) - 좌표를 순수 Python int로 변환
                corner_count = min(len(projected_front), len(projected_back)) if projected_front is not None and projected_back is not None else 0
                for i in range(corner_count):
                    pt1 = self._to_cv_point(projected_front[i])
                    pt2 = self._to_cv_point(projected_back[i])
            ...
            if self.vis_settings.get('plane1', True):
                ...
                plane1_poly = self._to_int_poly(projected_points)
                if plane1_poly is not None:
                    cv2.polylines(overlay_frame, [plane1_poly], True, (0, 255, 255), 3)
            ...
            if self.vis_settings.get('plane2', True):
                ...
                plane2_poly = self._to_int_poly(projected_points2)
                if plane2_poly is not None:
                    cv2.polylines(overlay_frame, [plane2_poly], True, (255, 100, 0), 3)
        ...
        # 판정 효과 (테두리 그라데이션) 그리기
        overlay_frame = self._draw_judgment_effect(overlay_frame)
    ...
    def _draw_grid_on_plane2(self, frame, rvec, tvec):
        ...
        for i in range(1, 3):
            ...
            pt1 = self._to_cv_point(pts[0])
            pt2 = self._to_cv_point(pts[1])
            if pt1 and pt2:
    ...
    def _draw_target_zone_on_plane2(self, frame, rvec, tvec, target_zone, color=(0, 165, 255), alpha=0.4):
        ...
        poly = self._to_int_poly(projected)
        if poly is None:
            return
    ...
    def _draw_judgment_effect(self, frame):
        """판정 효과 (테두리 그라데이션) 그리기"""
        if self.judgment_effect_type is None:
            return frame
        
        current_time = time.time()
        elapsed = current_time - self.judgment_effect_start_time
        
        # 효과 지속 시간 초과 시 종료
        if elapsed > self.judgment_effect_duration:
            self.judgment_effect_type = None
            return frame
        ...
        if self.game_result is not None:
            if self.game_result['success']:
                game_color = (0, 255, 0)  # 초록색 (성공)
                game_text = f"SUCCESS! Zone {self.game_result['actual']}"
            else:
                game_color = (0, 0, 255)  # 빨간색 (실패)
                if self.game_result['actual'] == 0:
                    game_text = f"MISS! (Outside)"
                else:
                    game_text = f"MISS! Zone {self.game_result['actual']}"
        
        # 색상 설정 (BGR) - 게임모드면 게임 색상 우선
        if game_color is not None:
            color = game_color
            text = game_text
        elif self.judgment_effect_type == 'strike':
            color = (0, 255, 0)       # 초록색 (스트라이크)
            text = "STRIKE!"
        elif self.judgment_effect_type == 'ball':
            color = (0, 255, 255)     # 노란색 (볼)
            text = "BALL"
        elif self.judgment_effect_type == 'out':
            color = (0, 0, 255)       # 빨간색 (아웃)
            text = "OUT!"
        else:
            return frame
        
        # 페이드 아웃 효과 (시간에 따라 투명도 감소)
        alpha = 1.0 - (elapsed / self.judgment_effect_duration)
        alpha = max(0.0, min(1.0, alpha))
        
        # 그라데이션 테두리 두께 (안쪽으로 갈수록 투명)
        border_thickness = 40
        ...
        for i in range(border_thickness):
            # 바깥쪽일수록 진하게 (그라데이션)
            gradient_alpha = (1.0 - i / border_thickness) * alpha
            intensity = int(255 * gradient_alpha)
            
            # 색상에 그라데이션 적용
            grad_color = (
                int(color[0] * gradient_alpha),
                int(color[1] * gradient_alpha),
                int(color[2] * gradient_alpha)
            )
            
            # 테두리 사각형 그리기
            cv2.rectangle(overlay, (i, i), (w - 1 - i, h - 1 - i), grad_color, 1)
        
        # 블렌딩
        cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
        
        # 중앙에 판정 텍스트 표시 (처음 0.5초 동안만)
        if elapsed < 0.5:
            font = cv2.FONT_HERSHEY_SIMPLEX
            font_scale = 2.5 if game_text else 3.0  # 게임모드는 텍스트가 길어서 약간 작게
            thickness = 5
    ...
    def _trigger_judgment_effect(self, effect_type, game_result=None):
        """판정 효과 트리거
        
        Args:
            effect_type: 'strike', 'ball', 'out'
            game_result: 게임모드 결과 {'success': bool, 'target': int, 'actual': int} 또는 None
        """
        self.judgment_effect_type = effect_type  # 'strike', 'ball', 'out'
        self.judgment_effect_start_time = time.time()
        self.game_result = game_result  # 게임모드 결과 저장
        print(f"[효과] {effect_type.upper()} 효과 시작")
        if game_result:
            result_text = "성공! ✓" if game_result['success'] else "실패 ✗"
            print(f"[게임모드] 타겟:{game_result['target']} → 실제:{game_result['actual']} = {result_text}")
    
    def _get_zone_from_position(self, x, z):
        """
        공의 X, Z 좌표로 9분할 구역 번호 계산 (1~9)
        스트라이크존 밖이면 0 반환
        ...
        """
        # 스트라이크존 경계
        x_min = float(np.min(self.ball_zone_corners[:, 0]))
        x_max = float(np.max(self.ball_zone_corners[:, 0]))
        z_min = float(np.min(self.ball_zone_corners[:, 2]))
        z_max = float(np.max(self.ball_zone_corners[:, 2]))
        ...
        col = min(2, int(x_ratio * 3))
        
        # Z 방향 (상→하: 0, 1, 2) - Z가 높을수록 상단
        z_ratio = (z_max - z) / (z_max - z_min)  # 높은 Z가 0 (상단)
        row = min(2, int(z_ratio * 3))
    ...
    def _check_crossing(self, filtered_point, rvec, tvec):
        ...
        if self.plane1_crossed and self.plane2_crossed and cooldown_passed:
            ...
            game_result = None
            if self.game_mode_enabled and self.target_zone is not None:
                actual_zone = self._get_zone_from_position(
                    float(point_on_plane2[0]), 
                    float(point_on_plane2[2])
                )
                game_result = {
                    'success': (actual_zone == self.target_zone),
                    'target': self.target_zone,
                    'actual': actual_zone
                }
            
            # 스코어보드 업데이트 및 판정 효과 트리거
            if is_strike:
                # 삼진 체크 (스트라이크 추가 전에)
                is_strikeout = (self.scoreboard.strike_count == 2)  # 3번째 스트라이크
                self.scoreboard.add_strike()
                if is_strikeout:
                    self._trigger_judgment_effect('out', game_result)  # 아웃 효과 (빨강)
                else:
                    self._trigger_judgment_effect('strike', game_result)  # 스트라이크 효과 (초록)
            else:
                self.scoreboard.add_ball()
                self._trigger_judgment_effect('ball', game_result)  # 볼 효과 (노랑)
...
class IntegratedMainWindow(MainWindow):
    ...
    def _apply_saved_settings(self):
        ...
        if self.pitch_analyzer.game_mode_enabled:
            ...
            # ControlPanel의 game_mode_cb 사용
            if hasattr(self, 'control_panel') and hasattr(self.control_panel, 'game_mode_cb'):
                self.control_panel.game_mode_cb.setChecked(True)
