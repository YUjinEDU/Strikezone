...
def save_pitch_clip(frames, out_path, fps=60):
    """프레임 리스트를 비디오 파일로 저장"""
    if not frames:
        return False
...
def main():
    ...
    if not cap_fps or cap_fps <= 1:
        cap_fps = 60.0  # 기본값
    seconds_buffer = 2.0  # 이벤트 전 2초 프레임 보관
    video_buffer = deque(maxlen=int(cap_fps * seconds_buffer))
                ...
                for rvec, tvec in zip(rvecs, tvecs):
                    ...
                    if center and radius > 0.4:
                        ...
                        if (t_cross_plane1 is not None) and (t_cross_plane2 is not None):
                            ...
                            save_pitch_clip(list(video_buffer), clip_path, fps=cap_fps)
                            print(f"✅ 영상 클립 저장: {clip_path}")

                            # ==== 대시보드에 피치 레코드 추가 ====
                            trajectory_points = list(zip(pitch_points_trace_3d_x, pitch_points_trace_3d_y, pitch_points_trace_3d_z))
                            point_3d = [float(filtered_point[0]), float(filtered_point[1]), float(filtered_point[2])]
                            ...
                            dashboard.update_data(dashboard_update_data)
